...
## メモ
* コンパイルの方針
* headerディレクトリ内の.cをすべてコンパイル
  * `gcc -c head/*.c -o head/*.c` 
* 生徒提出のプログラムは
  * `gcc -I head -o 01234567 01234567.c [header内の.o すべて]`
* 最終的な実行方法
  * `python3 main.py [課題のcが格納されたディレクトリ] -io [入出力があれば入出力のフォルダ] -h [ヘッダがあればheaderのディレクトリ]`
* 9/24 の暫定
  * `python3 main.py [課題のcが格納されたディレクトリ] -io [入出力があれば入出力のフォルダ] -h [ヘッダがあればheaderのディレクトリ]`
